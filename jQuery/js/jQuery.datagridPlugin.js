    ...
    var functions = {
        ...
        rows : function(config, table, data) {
            for(i=0; i < data.length; i++) {
                ...
                var functions = this;
                rowObj.click(function(event) { functions.rowClick(event, this); } );
                if(config.onDoubleclick != null) {
                    rowObj.dblclick( function(event) { config.onDoubleclick(event, this); } );
                }
        ...
        alignAttr : function(align) {
            if(align != null) return "align='" + align + "'";
            return "";
        }
        ,
        rowClick : function(event, selectedRow) {
            ...
            if(rowAlreadySelected){
                if(!rowAlreadyUnselected){
                    ...
                    if(config.onUnselect != null) config.onUnselect();
                }
            } else  {
                row.data("origClass", row.attr("class") );
                row.attr("class", config.selectedClass);
                if(config.onSelect != null) config.onSelect();
            }
        }
        ,
